...
export const IssuesToucheese: ToucheeseIssueType = [
  {
    title: "React 렌더링 사이클을 고려한 비동기 상태 업데이트 최적화",
  ...
  },
  {
    ...
    solution:
      "브라우저의  WebP  이미지 포멧 지원 여부를 감지하는 함수를 구현하여 최적화된 이미지 포멧을 로드",
    outcome:
      "평균 이미지 파일 크기를 321KB에서 211KB로 약 34% 감소시켜 페이지 로딩속도 향상 및 네트워크 비용 절감",
  },
  {
    title: "테스트코드를 통한 성능개선",
    imgAfter: "/img/toucheese-issue-05.webp",
    solution:
      "테스트 코드를 통해 비밀번호 검증에서 성능 저하를 발견하여 길이 검사를 먼저 수행하는 로직으로 수정",
    outcome:
      "불필요한 정규식 검사 비용을 줄여 실행 시간단축 및 성능 약 40% 개선",
  },
];
